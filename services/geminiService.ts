...
import { GoogleGenerativeAI, HarmCategory, HarmBlockThreshold, GenerationConfig, ResponseMimeType, FunctionDeclarationSchema, FunctionDeclaration, Part, Content } from "@google/generative-ai";
...
export async function analyzeThread(config: ScanConfig, images: string[]): Promise<AnalysisResult> {
  const imageParts: Part[] = images.map((base64Data) => {
  ...
  const prompt = `
    ...
    **OUTPUT FORMAT (Strict JSON):**
    ...
    - A 'measurement' object with the measurement of the thread.
    - A 'treadDimension' object with the tread dimension of the thread.
  `;
